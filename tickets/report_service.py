...
class LocalReportService:
    def __init__(self):
        self.local_service = LocalTicketService()
    
    def _get_all_tickets(self, start_date=None, end_date=None, limit=1000):
        """Get tickets from local database - super fast"""
        filters = {}
        if start_date:
        ...
        tickets = self.local_service.get_tickets_for_reports(filters)
    ...
    def get_monthly_report(self, year=None, month=None):
        """Get tickets for a specific month - optimized"""
        ...
        # Use smaller batch for monthly reports
        all_tickets = self.api.get_tickets(row_count=1500)
    ...
    def _process_report_data(self, tickets, report_type, start_date=None, end_date=None, 
        ...
        """Process tickets into report format - optimized"""
        ...
        for ticket in tickets:
            ...
            # Add ticket details (limit to 100 for performance)
            if len(report['tickets']) < 100:
                report['tickets'].append(self._format_ticket(ticket))
        
        report['status_breakdown'] = status_count
        report['priority_breakdown'] = priority_count
        report['daily_breakdown'] = dict(sorted(daily_count.items()))
