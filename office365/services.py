    ...
    def get_license_summary(self):
        ...
        for sku in data.get("value", []):
            ...
            if sku_name in ["O365_BUSINESS_PREMIUM", "O365_BUSINESS_ESSENTIALS"]:
                ...
                total = prepaid_units.get("enabled", 0) + prepaid_units.get("suspended", 0)
                consumed = sku.get("consumedUnits", 0)
                
                # Fix for cases where consumed > total (Microsoft API inconsistency)
                if consumed > total and total == 0:
                    total = consumed
